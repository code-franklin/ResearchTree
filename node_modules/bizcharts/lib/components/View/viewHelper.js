...
var ViewHelper = /** @class */ (function () {
    ...
    ViewHelper.prototype.update = function (newConfig) {
        ...
        if (data.rows) {
            (0, warning_1.default)(!data.rows, 'bizcharts@4不支持 dataset数据格式，请使用data={dv.rows}');
            data = data.rows;
        }
        if (!this.view || (0, is_array_1.default)(preData) && preData.length === 0) {
            // hack g2 数据切换的问题
            this.destroy();
            this.creatViewInstance(newConfig);
        }
        // 数据
        if ((0, is_array_1.default)(preData)) {
        ...
        (0, each_1.default)(filter, function (it, index) {
            if ((0, is_array_1.default)(it)) {
    ...
    ViewHelper.prototype.processOptions = function (options) {
        ...
        (0, warning_1.default)(!start, 'start 属性将在5.0后废弃，请使用 region={{ start: {x:0,y:0}}} 替代');
        (0, warning_1.default)(!end, 'end 属性将在5.0后废弃，请使用 region={{ end: {x:0,y:0}}} 替代');
        var regionCfg = (0, deep_mix_1.default)({ start: { x: 0, y: 0 }, end: { x: 1, y: 1 } }, { start: start, end: end }, region);
